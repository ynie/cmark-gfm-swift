...
const char *cmark_node_get_hashtag_issue_number(cmark_node *node) {
...
static cmark_node *match(cmark_syntax_extension *self, cmark_parser *parser,
    ...
    while (end < size && cmark_isdigit(data[end])) {
        end++;
    }
    ...
    cmark_chunk *hashtag_chunk;
    node->as.opaque = hashtag_chunk = parser->mem->calloc(1, sizeof(cmark_chunk));
    hashtag_chunk->data = data + at;
    hashtag_chunk->len = end - at;
...
static void html_render(cmark_syntax_extension *extension,
    ...
    const char *hashtag = cmark_node_get_hashtag_issue_number(node);
    if (hashtag == NULL) {
        return;
    }
    ...
    cmark_strbuf_puts(html, "<a href=\"https://github.com/[[REPO_URL_PART]]/");
    cmark_strbuf_puts(html, hashtag);
...
static const char *get_type_string(cmark_syntax_extension *extension,
                                   cmark_node *node) {
    return node->type == CMARK_NODE_MENTION ? "hashtag" : "<unknown>";
}
